...
class RRTConnect(rrt.RRT):
    ...
    def _smooth_path(self,
                     ...
                     obstacle_list=[],
                     otherrobot_list=[],
                     granularity=2, # 插值步长(关节空间的最大允许移动距离)
                     iterations=50, # 平滑迭代次数
                     animation=False):  # 是否启用可视化动画
        smoothed_path = path
