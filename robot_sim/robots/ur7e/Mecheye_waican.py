...
class Mech_Calibration: # 梅卡曼德相机标定
...
class ControlWindow:
    def __init__(self, root, camera_calibration):
        self.camera_calibration = camera_calibration
        self.camera_calibration.control_window = self  # 建立反向引用
        self.root = root
        ...
        self.sensitivity_label = tk.Label(sensitivity_frame, text=f"{self.camera_calibration.sensitivity_multiplier:.1f}x",
                                          font=("Arial", 12, "bold"), fg="blue")
        self.sensitivity_label.pack(side=tk.LEFT, padx=10)

        # 添加灵敏度调节按钮
        tk.Button(sensitivity_frame, text="降低灵敏度 (X)",
                  command=self.camera_calibration.decrease_sensitivity).pack(side=tk.LEFT, padx=5)
        tk.Button(sensitivity_frame, text="提高灵敏度 (空格)",
                  command=self.camera_calibration.increase_sensitivity).pack(side=tk.LEFT, padx=5)
        ...
        tk.Button(button_frame, text="输出数值", command=self.camera_calibration.print_final_values).pack(side=tk.LEFT, padx=5)

    def update_sensitivity_display(self):
        """更新灵敏度显示"""
        self.sensitivity_label.config(text=f"{self.camera_calibration.sensitivity_multiplier:.1f}x")
    ...
    def update_point_cloud(self):
        ...
        # 同步更新内部状态
        self.camera_calibration.current_rotation = [alpha, beta, gamma]
        self.camera_calibration.current_translation = [tx, ty, tz]

        self.camera_calibration.update_point_cloud(alpha, beta, gamma, tx, ty, tz)

    def update_sliders_from_values(self):
        """从内部状态更新滑块值"""
        # 更新滑块和输入框
        self.alpha_slider.set(self.camera_calibration.current_rotation[0])
        self.beta_slider.set(self.camera_calibration.current_rotation[1])
        self.gamma_slider.set(self.camera_calibration.current_rotation[2])
        self.tx_slider.set(self.camera_calibration.current_translation[0])
        self.ty_slider.set(self.camera_calibration.current_translation[1])
        self.tz_slider.set(self.camera_calibration.current_translation[2])

        # 更新输入框
        self.alpha_entry.delete(0, tk.END)
        self.alpha_entry.insert(0, f"{self.camera_calibration.current_rotation[0]:.1f}")
        self.beta_entry.delete(0, tk.END)
        self.beta_entry.insert(0, f"{self.camera_calibration.current_rotation[1]:.1f}")
        self.gamma_entry.delete(0, tk.END)
        self.gamma_entry.insert(0, f"{self.camera_calibration.current_rotation[2]:.1f}")
        self.tx_entry.delete(0, tk.END)
        self.tx_entry.insert(0, f"{self.camera_calibration.current_translation[0]:.3f}")
        self.ty_entry.delete(0, tk.END)
        self.ty_entry.insert(0, f"{self.camera_calibration.current_translation[1]:.3f}")
        self.tz_entry.delete(0, tk.END)
        self.tz_entry.insert(0, f"{self.camera_calibration.current_translation[2]:.3f}")

    def reset_transform(self):
        """重置所有变换"""
        self.camera_calibration.current_rotation = [0, 0, 0]
        self.camera_calibration.current_translation = [0, 0, 0]
        self.update_sliders_from_values()
        self.camera_calibration.update_point_cloud_from_keyboard()
...
if __name__ == '__main__':
    ...
    # 初始化 Panda3D 显示窗口
    mech_calibration = Mech_Calibration(root)

    # 加载点云路径
    pcd_path = r'E:\py_project\wrsrobot\wrs_shu\yanpu_ur8\images\Mech\pointcloud.ply'

    mech_calibration.add_point_cloud(pcd_path)

    # 创建控制窗口
    control_window = ControlWindow(root, mech_calibration)

    # 主线程运行 Panda3D 窗口
    mech_calibration.run_panda3d()
...
4x4 变换矩阵:
  [0.048849, 0.998803, 0.002674, 0.179000]
  [-0.998789, 0.048832, 0.006072, 0.360000]
  [0.005934, -0.002967, 0.999978, 1.990000]
