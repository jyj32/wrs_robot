...
class GOFA12(mi.ManipulatorInterface):
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), homeconf=np.zeros(6), name='gofa12', enable_cc=True):
        ...
        self.jlc.jnts[1]['loc_pos'] = np.array([0, 0, 0.214])#平移矩阵
        self.jlc.jnts[1]['loc_motionax'] = np.array([0,0, 1])#旋转轴

        self.jlc.jnts[2]['loc_pos'] = np.array([0, -0.09515, 0.124])#平移矩阵
        self.jlc.jnts[2]['loc_rotmat'] = rm.rotmat_from_euler(-np.pi/2, 0, np.pi)#旋转矩阵
        self.jlc.jnts[2]['loc_motionax'] = np.array([0,0, 1])#旋转轴

        self.jlc.jnts[3]['loc_pos'] = np.array([0,-0.707 , 0.01015])
        self.jlc.jnts[3]['loc_rotmat'] = np.dot(rm.rotmat_from_axangle([0, 0, 1], np.pi*1/2),rm.rotmat_from_axangle([1, 0, 0], np.pi))#旋转矩阵
        self.jlc.jnts[3]['loc_motionax'] = np.array([0,0,1])

        self.jlc.jnts[4]['loc_pos'] = np.array([-0.11,-0.096 ,0.085])
        self.jlc.jnts[4]['loc_rotmat'] = rm.rotmat_from_euler(np.pi/2, 0, 0)
        self.jlc.jnts[4]['loc_motionax'] = np.array([0, 0, 1])

        self.jlc.jnts[5]['loc_pos'] = np.array([0, 0.0755, 0.437968])
        self.jlc.jnts[5]['loc_rotmat'] = rm.rotmat_from_euler(np.pi/2, np.pi/2, 0)
        self.jlc.jnts[5]['loc_motionax'] = np.array([0, 0, 1])

        self.jlc.jnts[6]['loc_pos'] = np.array([-0.068032, -0.08, 0.0755])     
        self.jlc.jnts[6]['loc_rotmat'] = np.dot(rm.rotmat_from_euler(-np.pi/2,0, 0),
                                                rm.rotmat_from_axangle([0, 1, 0], -np.pi/2) ) # 发生自锁，先旋转后一个旋转矩阵，再旋转前一旋转矩阵
