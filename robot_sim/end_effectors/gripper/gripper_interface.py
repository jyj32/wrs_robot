...
class GripperInterface(object):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), cdmesh_type='aabb', name='gripper'):
        ...
        # jaw center
        self.jaw_center_pos = np.zeros(3)   # 夹爪中心相对于手爪坐标系的位移，无平移
        self.jaw_center_rotmat = np.eye(3)  # 夹爪中心相对于手爪坐标系的旋转矩阵，无旋转，朝向一致
        # jaw width
    ...
    def grip_at_with_jcpose(self, gl_jaw_center_pos, gl_jaw_center_rotmat, jaw_width):
        ...
        输入：
            gl_jaw_center_pos:期望的夹爪中心相对于世界坐标系的位置（要抓的物体的位置）
            gl_jaw_center_rotmat: 期望的夹爪中心相对于世界坐标系的旋转矩阵（要抓的物体的旋转矩阵）
            jaw_width:期望的夹爪宽度
        return:
            jaw_width:手爪宽度
            gl_jaw_center_pos:期望的夹爪中心相对于世界坐标系的位置（要抓的物体的位置）
            gl_jaw_center_rotmat:期望的夹爪中心相对于世界坐标系的旋转矩阵（要抓的物体的旋转矩阵）
            eef_root_pos:末端执行器相对于世界坐标系的位置
            eef_root_rotmat:末端执行器相对于世界坐标系的旋转矩阵
        """
        self.jaw_to(jaw_width)
        # 末端执行器在世界坐标系中的旋转矩阵=期望的夹爪中心相对于世界坐标系的旋转矩阵（要抓的物体的旋转矩阵）*夹爪中心相对于末端执行器坐标系的旋转矩阵再转置
        eef_root_rotmat = gl_jaw_center_rotmat.dot(self.jaw_center_rotmat.T)
        # 末端执行器相对于世界坐标系的位置 = 期望的夹爪中心相对于世界坐标系的位置（要抓的物体的位置）- 末端执行器相对于世界坐标系的旋转矩阵*夹爪中心相对于末端执行器坐标系的位置（相乘结果为"在世界坐标系下，从末端执行器原点指向夹爪中心的向量"）
        eef_root_pos = gl_jaw_center_pos - eef_root_rotmat.dot(self.jaw_center_pos)
        # 将机械臂的末端执行器移动到计算出的位姿（位置 + 朝向）
