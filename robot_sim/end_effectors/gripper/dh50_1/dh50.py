...
class Dh50(gp.GripperInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), coupling_offset_pos=np.zeros(3),
        ...
        self.lft.jnts[1]['loc_pos'] = np.array([0, 0.014087, 0.1688])
        self.lft.jnts[1]['type'] = 'prismatic'
        self.lft.jnts[1]['loc_rotmat'] = rm.rotmat_from_euler(0, 0, np.pi/2)
        self.lft.jnts[1]['motion_rng'] = [0, .025]   # 位置范围
        self.lft.jnts[1]['loc_motionax'] = np.array([1, 0, 0])  # 关节的移动方向
        ...
        self.lft.lnks[1]['mesh_file'] = cm.CollisionModel(
            ...
            userdefined_cdprimitive_fn=self._lftfinger_cdnp, expand_radius=.000)    # 自定义碰撞体
        ...
        self.rgt.jnts[1]['loc_pos'] = np.array([0, -0.014087, 0.1688])
        self.rgt.jnts[1]['type'] = 'prismatic'
        self.rgt.jnts[1]['loc_motionax'] = np.array([-1, 0, 0])  # 关节的移动方向
        self.rgt.jnts[1]['loc_rotmat'] = rm.rotmat_from_euler(0, 0, np.pi / 2)
        ...
        self.rgt.lnks[1]['mesh_file'] = cm.CollisionModel(
            ...
            userdefined_cdprimitive_fn=self._rgtfinger_cdnp, expand_radius=.000)    # 自定义碰撞体
    ...
    @staticmethod
    def _lftfinger_cdnp(name, radius):  # 自定义左手抓碰撞体
        collision_node = CollisionNode(name)
        collision_primitive_c0 = CollisionBox(Point3(0, 0, -.035),   # 碰撞体中心点
                                              x=.007 + radius, y=0.014 + radius, z=0.035 + radius)   # 碰撞体尺寸
        collision_node.addSolid(collision_primitive_c0)
        # 新增碰撞盒，在手指两边
        collision_primitive_c1 = CollisionBox(Point3(0.019, 0, -.06),
                                              x=.012 + radius, y=0.014 + radius, z=0.01 + radius)  # 尺寸
        collision_node.addSolid(collision_primitive_c1)
        return collision_node
    @staticmethod
    def _rgtfinger_cdnp(name, radius):  # 自定义右手抓碰撞体
        collision_node = CollisionNode(name)
        collision_primitive_c0 = CollisionBox(Point3(0, 0, -.035),
                                              x=.007 + radius, y=0.014 + radius, z=0.035 + radius)
        collision_node.addSolid(collision_primitive_c0)
        # 新增碰撞盒，在手指两边
        collision_primitive_c1 = CollisionBox(Point3(-0.019, 0, -.06),
                                              x=.012 + radius, y=0.014 + radius, z=0.01 + radius)  # 尺寸
