...
class Dh50(gp.GripperInterface):
    ...
    def __init__(self, pos=np.zeros(3), rotmat=np.eye(3), coupling_offset_pos=np.zeros(3),
        ...
        # 左手爪
        self.lft.jnts[1]['loc_pos'] = np.array([0.013, 0.0065, .1025]) # 位置
        self.lft.jnts[1]['loc_rotmat'] = rm.rotmat_from_euler(0, 0, np.pi / 2)  # 旋转
        self.lft.jnts[1]['type'] = 'prismatic'
        self.lft.jnts[1]['motion_rng'] = [0, .025]
        self.lft.jnts[1]['loc_motionax'] = np.array([1, 0, 0])  # 运动方向
        ...
        self.rgt.jnts[1]['loc_pos'] = np.array([-0.013, -0.0065, .1025])  # 位置
        self.rgt.jnts[1]['loc_rotmat'] = rm.rotmat_from_euler(0, 0, np.pi / 2)  # 旋转
