...
if __name__ == '__main__':
    ...
    start_conf = np.array([0.0439823, -0.53023103, 1.05243354, 0.0143117, 1.55351757, 1.57079633])  # 起始关节角度
    # print(start_conf)
    hand_name = "hnd"  # 手爪
    # object放置初始物体
    objcm_name = "123"  # 物体
    current_dir = os.path.dirname(os.path.abspath(__file__))  # 当前目录
    stl_path = os.path.join(current_dir, "123.STL")
    # 然后使用这个路径
    obj = cm.CollisionModel(stl_path)  # 创建碰撞模型
    # 初始物体
    obj.set_rgba([.9, .75, .35, 1])  # 颜色
    obj.set_pos(np.array([.4, -.4, 0.1]))  # 开始位置
    obj.set_rotmat(rotmat_x_90)  # 开始姿态
    obj.show_localframe()
    obj.attach_to(base)
    ...
    obj_goal.set_pos(np.array([.4, .4, 0.1]))  # 目标位置
    obj_goal.set_rotmat(rotmat_x_90)  # 目标姿态
    obj_goal.attach_to(base)

    # base.run()

    gripper_s = dh.Ag145()  # 创建手爪实例
    grasp_info_list = gpa.load_pickle_file(objcm_name, root=current_dir,
                                           file_name='123_AG145_6.pickle')  # 从pickle文件中加载预计算抓取姿态
