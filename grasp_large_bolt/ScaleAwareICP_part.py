...
class RotationOnlyICP:
    ...
    def visualize_final_result(self, scene, model, transformation):
        ...
        if self.cluster_bbox_center is not None:
            bbox_center_marker = self.create_center_marker(
                self.cluster_bbox_center, size=0.005, color=[1, 0, 1]
            )
    ...
    def simple_icp_rotation_only(self, scene, model, max_distance=0.02,
        ...
        # 步骤1：下采样
        scene_down = scene.voxel_down_sample(0.001) # 0.005
        model_down = model.voxel_down_sample(0.001)
    ...
    def simple_icp_with_scale_fix(self, ply, ply_path, stl_path, manual_scale=None, scene_down_num = 0.001,
                                use_cluster_center=True, use_bbox_center=True, eps=0.02,
                                icp_iteration = 200, min_samples=30, max_distance = 0.02,
                                show = False):
        """
        计算平移和旋转对齐，进行迭代。
        输入：
        ...
        scene_down = scene.voxel_down_sample(scene_down_num)
        model_down = scaled_model_cloud.voxel_down_sample(scene_down_num)
        ...
        result1 = o3d.pipelines.registration.registration_icp(
            ...
            o3d.pipelines.registration.ICPConvergenceCriteria(  # 迭代终止条件
                max_iteration=icp_iteration,  # 最大迭代次数
                relative_fitness=1e-10,  # 连续两次迭代之间适应度
                relative_rmse=1e-10) # 均方根误差的相对变化
        )
        print(f"\nICP1结果:")
        print(f"  匹配度: {result1.fitness:.4f}")   # 范围 [0,1]，越大越好
        print(f"  误差: {result1.inlier_rmse:.6f}")    # 越小越好
        T_init2 = T_init.copy()
        T_init2[:3, :3] = rm.rotmat_from_axangle(np.array([0, 0, 1]), np.pi) @ T_init[:3, :3]
        ...
        print(f"\nICP2结果:")
        print(f"  匹配度: {result2.fitness:.4f}")  # 范围 [0,1]，越大越好
        print(f"  误差: {result2.inlier_rmse:.6f}")  # 越小越好
        if result2.inlier_rmse < result1.inlier_rmse:
            transformation = result2.transformation
        elif result2.inlier_rmse > result1.inlier_rmse:
            transformation = result1.transformation
        else:   # 误差相同
            if result2.fitness < result1.fitness:   # 比较匹配度
                transformation = result1.transformation
            else:
                transformation = result2.transformation
...
if __name__ == "__main__":
    PLY_PATH = r"E:\py_project\wrsrobot\wrs_shu\grasp_large_bolt\images\Mech\pointcloud.ply"
    STL_PATH = r"/grasp_large_bolt/object/M16hole.STL"
    ply = None
    ...
    # 使用包围盒中心对齐（推荐）
    transformation = icp.simple_icp_with_scale_fix( ply, PLY_PATH, STL_PATH, manual_scale=None,
