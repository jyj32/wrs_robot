...
if __name__ == '__main__':
    ...
    objpath = os.path.join(this_dir, 'U625.STL') #物体路径：当前文件夹下
    # 检查文件是否存在
    ...
    jaw_center_pos = np.array([0, 0, 0.01])    # 手爪中心位置
    jaw_center_rotmat1 = rm.rotmat_from_axangle([0, 1, 0], math.pi) # 手爪中心的旋转矩阵
    hnd_pos = np.array([0, 0, 0])    # 手爪坐标系偏移位置
    hnd_rotmat = np.eye(3)  # ?
    jaw_center_rotmat2 =rm.rotmat_from_axangle([0, 0, 1], math.pi/4) @ jaw_center_rotmat1  # 手爪中心的旋转矩阵
    jaw_center_rotmat3 = rm.rotmat_from_axangle([0, 0, 1], -math.pi / 4) @ jaw_center_rotmat1  # 手爪中心的旋转矩阵
    jaw_center_rotmat4 = rm.rotmat_from_axangle([0, 0, 1], math.pi /2) @ jaw_center_rotmat1  # 手爪中心的旋转矩阵
    ...
    grasp_info4 = [jaw_width, jaw_center_pos, jaw_center_rotmat4, hnd_pos, hnd_rotmat]
    grasp_info_list.append(grasp_info1)
    grasp_info_list.append(grasp_info2)
    grasp_info_list.append(grasp_info3)
    grasp_info_list.append(grasp_info4)

    gpa.write_pickle_file('U625', grasp_info_list, './', filename)
